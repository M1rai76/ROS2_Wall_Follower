...
void WallFollower::update_callback()
{
	...
	if (near_start) {
    ...
    else if (scan_data_[FRONT_LEFT] > scan_data_[LEFT_FRONT]) {
		update_cmd_vel(0, 0.1);

		std::string print2 = "update_cmd_vel(0, 0.1)\n";
		RCLCPP_INFO(this->get_logger(), print2);

	} else if (scan_data_[FRONT_LEFT] > scan_data_[FRONT] > scan_data_[FRONT_RIGHT]){
		update_cmd_vel(0.1, 0);

		std::string print4 = "update_cmd_vel(0.1, 0)\n";
		RCLCPP_INFO(this->get_logger(), print4);

	} 
	else if (scan_data_[FRONT] < 0.2){
		update_cmd_vel(0, -0.1);
		std::string print4 = "update_cmd_vel(0, -0.1)\n";
		RCLCPP_INFO(this->get_logger(), print4);
	}
	else if (scan_data_[FRONT_LEFT] > scan_data_[LEFT] and scan_data_[FRONT_RIGHT] >scan_data_[LEFT]){
		update_cmd_vel(0, -0.1);
		std::string print4 = "update_cmd_vel(0, -0.1)\n";
		RCLCPP_INFO(this->get_logger(), print4);
	}
		...
		std::string print7 = "Here 7 (should be JUST moving forward)\n";
	...
	for (int i = 0; i < 12; i++)
	{
		...
		scan_data_i = scan_data_i + std::to_string(scan_data_[i]) + " ";
	}

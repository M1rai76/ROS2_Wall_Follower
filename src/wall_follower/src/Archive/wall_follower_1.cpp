...
void WallFollower::update_callback()
{
	...
	{
		int angle = (i) * 30;
		angle_i = angle_i + std::to_string(angle) + " \n";
		scan_data_i = scan_data_i + std::to_string(i) + " " + std::to_string(scan_data_[i]) + " \n";
	}

	if (near_start) {
		update_cmd_vel(0.0, 0.0);

		std::string print1 = "NEAR_START1 (shouldn't be doing anything)\n";
		RCLCPP_INFO(this->get_logger(), print1);
	}

    else if ((scan_data_[FRONT_LEFT] > scan_data_[LEFT_FRONT]) and ((scan_data_[FRONT_LEFT] > 0.5) and (scan_data_[LEFT_FRONT] > 0.5) and  (scan_data_[LEFT] > 0.5) and (scan_data_[BACK_LEFT] > 0.5) and (scan_data_[LEFT_BACK] > 0.5))){
		update_cmd_vel(0, 0.1);

		std::string print2 = "Turning left towards wall. update_cmd_vel(0, 0.1) robot_pose_: " + std::to_string(robot_pose_) + "\n";
		RCLCPP_INFO(this->get_logger(), print2);
	} 

	else if ((robot_pose_ > 0.79) and (scan_data_[LEFT_FRONT] > 0.25)){
		update_cmd_vel(0.1, 0);

		std::string print2 = "Go towards the wall - update_cmd_vel(0.1, 0)\n";
		RCLCPP_INFO(this->get_logger(), print2);

	}

	else if ((scan_data_[LEFT_FRONT] < 0.25) and (robot_pose_ > 0.0)){
		update_cmd_vel(0.0, -0.1);
		std::string print2 = "Turning right parallel to the wall - update_cmd_vel(0.0, -0.1)\n";
		RCLCPP_INFO(this->get_logger(), print2);
		...
		update_cmd_vel(0.0, 0.0);
		std::string print4 = "Final else block update_cmd_vel(0.0, 0.0)robot_pose_: " + std::to_string(robot_pose_) + "\n";
